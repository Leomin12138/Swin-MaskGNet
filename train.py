...
from backbone.swin_transformer import swin_t, Swin_T_Weights
from backbone.swin_transformer_fpn_model import BackboneWithFPN
from torchvision.ops.feature_pyramid_network import LastLevelMaxPool


def create_model(num_classes, load_pretrain_weights=True):

    swin_backbone = swin_t(weights=Swin_T_Weights.DEFAULT).features  # 这个函数是获取swin_T的特征提取模型并以官方权重进行初始化
    # swin_backbone = swin_t(weights=None).features  # 这个函数是获取swin_T的特征提取模型并以官方权重进行初始化

    print([name for name, _ in swin_backbone.named_children()])

    return_nodes = {'1': '0', '3': '1', '5': '2', '7': '3'}  # 这个函数的作用是利用 backbone.named_children()得到此时放回的子模块层数
    # 并以 ['0', '1', '2', '3', '4', '5', '6', '7']表示名称 然后提取出 想要的特征层 {'1': '0', '3': '1', '5': '2', '7': '3'}
    # 形成新的特征层

    backbone = BackboneWithFPN(swin_backbone,
                               return_layers=return_nodes,  # 一个字典，指定了要从backbone中提取哪些层的输出，以及输出的名称。
                               in_channels_list=[96, 96 * 2, 96 * 2 * 2, 96 * 2 * 2 * 2],  # 对应上面四个层的输出通道
                               out_channels=256,  # FPN的输出通道
                               extra_blocks=LastLevelMaxPool())

    model = MaskRCNN(backbone, num_classes=num_classes, angle_num_classes=18)  # 抓取分支已经加入到了 mask rcnn的bbox分支 上面 共用一个 ROI
...
def main(args):
    ...
    # define optimizer
    # 定义优化器，只包括那些requires_grad=True的参数
    params = [p for p in model.parameters() if p.requires_grad]
    if len(params) > 0:  # 如果有参数需要更新
