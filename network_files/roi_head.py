...
class RoIHeads(torch.nn.Module):
    ...
    def select_grasp_training_samples(self,
        ...
        # 获取标注好的boxes以及labels信息
        gt_boxes = [t["grasps"].to(dtype) for t in targets]
        gt_labels = [t["angle_labels"] for t in targets]
    ...
    def forward(self,
        ...
        if self.training:
            ...
            grasp_matched_idxs = None
            grasp_labels = None
            grasp_regression_targets = None

        # 将采集样本通过Multi-scale RoIAlign pooling层
        # box_features_shape: [num_proposals, channel, height, width]
        box_features = self.box_roi_pool(features, proposals, image_shapes)
        grasp_feature = self.box_roi_pool(features, grasp_proposals, image_shapes)

        # 通过roi_pooling后的两层全连接层
        # box_features_shape: [num_proposals, representation_size]
        grasp_feature = self.grasp_box_head(grasp_feature)
        box_features = self.box_head(box_features)

        # 接着分别预测目标类别和边界框回归参数
        class_logits, box_regression = self.box_predictor(box_features)
        grasp_class_logits, grasp_box_regression = self.grasp_box_predictor(grasp_feature)
        ...
        if self.training:
            ...
            grasp_loss_classifier, grasp_loss_box_reg = fastrcnn_loss(
                grasp_class_logits, grasp_box_regression, grasp_labels, grasp_regression_targets)

            losses = {
                "loss_classifier": loss_classifier,
                "loss_box_reg": loss_box_reg,
                "grasp_loss_classifier": grasp_loss_classifier,
                "grasp_loss_box_reg": grasp_loss_box_reg
            }
        else:
                ...
                result.append(
                    {
                        ...
                        "labels": labels[i],
                        "scores": scores[i],
                        "grasp_boxes": None,  # 默认填None，等抓取检测填充
                        "grasp_labels": None,
                        "grasp_scores": None,
                    }
                )
            ...
            for i in range(len(grasp_boxes)):
                if i < len(result):
                    # 如果已经有普通检测结果，补充抓取检测的结果
                    result[i]["grasp_boxes"] = grasp_boxes[i]
                    result[i]["grasp_labels"] = grasp_labels[i]
                    result[i]["grasp_scores"] = grasp_scores[i]
                else:
                    # 如果没有普通检测结果，则新建一个条目
                    result.append(
                        {
                            ...
                            "labels": None,
                            "scores": None,
                            "grasp_boxes": grasp_boxes[i],
                            "grasp_labels": grasp_labels[i],
                            "grasp_scores": grasp_scores[i],
                        }
                    )
